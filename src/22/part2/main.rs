...
fn disintegrate(
    in_edges: &HashMap<Brick, HashSet<Brick>>,
    out_edges: &HashMap<Brick, HashSet<Brick>>,
    start: &Brick,
) -> usize {
    let mut queue: VecDeque<Brick> = VecDeque::from([*start]);
    let mut removed: HashSet<Brick> = HashSet::new();
...
fn main() {
    ...
    let mut ans = 0;
    for brick in &bricks {
        ans += disintegrate(&in_edges, &out_edges, brick);
    }
