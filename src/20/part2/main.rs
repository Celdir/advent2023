...
fn button(modules: &mut HashMap<String, Module>, monitored: &[&str]) -> Vec<String> {
    ...
    loop {
        ...
        if monitored.contains(&receiver.as_str()) && pulse == Pulse::Low {
            hits.push(receiver.clone());
        }
...
fn main() {
    ...
    for i in 1..5000 {
        for hit in button(&mut modules, &["ln", "dr", "zx", "vn"]) {
            if !cycles.contains_key(&hit) {
                cycles.insert(hit, i);
            }
        }
    }
    assert_eq!(cycles.len(), 4);
    let ans = cycles["ln"] * cycles["dr"] * cycles["zx"] * cycles["vn"];
    println!("{}", ans);
}
